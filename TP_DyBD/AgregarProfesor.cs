...
namespace TP_DyBD
{
    public partial class AgregarProfesor : Form
    {
        ...
        public AgregarProfesor()
        {
            ...
            cmb_cod_titulo.DataSource = SQL.ExecuteQuery("SELECT * FROM TITULOS");
            cmb_cod_titulo.DisplayMember = "desc_titulo";
            cmb_cod_titulo.ValueMember = "cod_titulo";
            cmb_cod_titulo.DropDownStyle = ComboBoxStyle.DropDownList;
            cmb_cod_doc.SelectedIndex = 0;
        }
        ...
        private void AgregarProfesor_Load(object sender, EventArgs e)
        {
            if (legajoSeleccionado.HasValue)
            {
                ...
                var profesor = SQL.ExecuteQuery($"SELECT * FROM Profesores WHERE nro_legajo_p = {legajoSeleccionado}");
                //mapear el profesor
                var leg = profesor.Rows[0]["nro_legajo_p"].ToString();
                var apenom = profesor.Rows[0]["ape_nom"].ToString();
                var ndoc = profesor.Rows[0]["nro_doc"].ToString();
                var direc = profesor.Rows[0]["direccion"].ToString();
                var email = profesor.Rows[0]["email"].ToString();
                var tel = profesor.Rows[0]["telefono"].ToString();
                var coddoc = profesor.Rows[0]["cod_doc"].ToString();
                var sexo = profesor.Rows[0]["sexo"].ToString();
                var nac = profesor.Rows[0]["fec_nac"].ToString();
                var estciv = profesor.Rows[0]["est_civil"].ToString();
                var codtit = profesor.Rows[0]["cod_titulo"].ToString();
                ...
                cmb_cod_titulo.SelectedValue = codtit;
        ...
        private void ModificarProfesor()
        {
            ...
            var cod_tit = cmb_cod_titulo.Text.ToString();

            try
            {
                SQL.ExecuteNonQuery($"UPDATE Profesores SET ape_nom = '{apenom}', nro_doc = {ndoc}, cod_doc = '{coddoc}', direccion = '{direc}', email = '{email}', telefono = '{tel}', sexo = '{sexo}', fec_nac = '{nac}', est_civil = '{estciv}' WHERE nro_legajo_p = {legajo}");
        ...
        private void InsertarProfesor()
        {
            ...
            var nac = dtp_fec_nac.Value.ToString("yyyy-dd-MM");
            var estciv = cmb_est_civil.Text.ToString();
            var cod_tit = cmb_cod_titulo.SelectedValue.ToString();

            try
            {
                SQL.ExecuteNonQuery($"INSERT INTO Profesores VALUES ({legajo}, '{apenom}', {ndoc}, '{coddoc}', '{direc}', '{email}', '{tel}', '{sexo}', '{nac}', '{estciv}', '{cod_tit}')");
